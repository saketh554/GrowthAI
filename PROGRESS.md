...
- 2026-06-10 - Part 9.5 - added live URL validation automation and docs: created `eval/verify_live_url.py` (health, frontend root, employees, submission creation, upload+verdict, submission detail, override, out-of-scope QA refusal checks) and wired usage into `docs/DEPLOYMENT.md` + `README.md`; deployment CLI execution was not possible in this environment (Render/Railway/GitHub CLIs unavailable), so this slice delivers a one-command post-deploy verifier - verified with `uv run python eval/verify_live_url.py --help` and lint checks on touched files - (not committed)
- 2026-06-10 - Part 9.4 - overhauled `README.md` to match brief deliverables: architecture overview + mermaid diagram, tradeoff rationale (retrieval/chunking, model tiering, vision usage, confidence and refusal policy), local/Docker/deploy instructions, eval harness usage, rough cost model, and 10k/day scaling path - verified by running documented harness command `uv run python eval/run.py --expected eval/expected.sample.json` and confirming successful metrics output - (not committed)
- 2026-06-10 - Part 9.3 - added deployment configuration and runbook: `render.yaml` (Docker web service, `/api/health` check, persistent disk at `/app/runtime`, runtime env-path mapping) and `docs/DEPLOYMENT.md` (Render/Railway steps, env vars, persistence notes); linked deployment section from `README.md` - verified by running container with deployment env overrides (`SQLITE_PATH`, `CHROMA_PATH`, `UPLOADS_DIR`, `POLICIES_DIR`) and confirming health endpoint returns 200 - (not committed)
- 2026-06-10 - Part 9.2 - added containerization baseline with multi-stage `Dockerfile` (frontend build + python app runtime) and `.dockerignore`; image runs uvicorn with PORT override and includes built frontend artifacts for backend static serving - verified with `docker build -t northwind-pre-review:part9 .` and container smoke check (`GET http://127.0.0.1:8000/api/health` returns 200/ok) - (not committed)
